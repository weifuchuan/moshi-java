...
public class MetaBuilder {
  ...
  protected void buildForeignKey(TableMeta tableMeta) throws SQLException {
    ...
    //		// 无主键的 table 将在后续的 removeNoPrimaryKeyTable() 中被移除，不再抛出异常
    //		// if (StrKit.isBlank(primaryKey)) {
    //		// throw new RuntimeException("primaryKey of table \"" + tableMeta.name + "\" required by
    // active record pattern");
    //		// }
    //
    //		tableMeta.primaryKey = primaryKey;
    //		rs.close();
    ResultSet rs = dbMeta.getImportedKeys(conn.getCatalog(), null, tableMeta.name);
    System.out.println(tableMeta.name);
    String fk = "";
    int index = 0;
    while (rs.next()) {
      String f = rs.getString("FKCOLUMN_NAME");
      if (index++ > 0) {
        fk += ",";
      }
      fk += f;
    }
    tableMeta.foreignKey = StrKit.isBlank(fk) ? null : fk;
    rs.close();
  }
   ...
   *
   * <p>社区分享：《Oracle NUMBER 类型映射改进》http://www.jfinal.com/share/1145
   */
