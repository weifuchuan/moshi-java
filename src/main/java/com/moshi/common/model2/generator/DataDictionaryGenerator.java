...
public class DataDictionaryGenerator {
	...
	*/
	...
	protected void genColumn(TableMeta tableMeta, ColumnMeta columnMeta, StringBuilder ret) {
	  ...
	  void rebuildColumnMetas(List<TableMeta> tableMetas) {
				...
				while (rs.next()) {
					...
					for (String key : keys) {
						if (key.equalsIgnoreCase(columnMeta.name)) {
							...
							columnMeta.isPrimaryKey2=true;
							break;
						}
					}

					if(tableMeta.foreignKey!=null){
					keys = tableMeta.foreignKey.split(",");
					for (String key : keys) {
						if (key.equalsIgnoreCase(columnMeta.name)) {
	 ...
	 */
	public void writeToFile(String ret) {
