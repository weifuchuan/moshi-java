...
class _Generator {
  ...
  fun main() {
    val prop = use("config-dev.properties")
    val druidPlugin = getDruidPlugin(prop)
    druidPlugin.start()
    val dataSource = druidPlugin.dataSource

    // base com.moshi.common.model 所使用的包名
    val baseModelPackageName = "com.moshi.common.com.moshi.common.model.base"
    // base com.moshi.common.model 文件保存路径
    val baseModelOutputDir = PathKit.getWebRootPath() + "/src/main/java/com/moshi/common/com.moshi.common.model/base"

    println("输出路径：$baseModelOutputDir")

    // com.moshi.common.model 所使用的包名 (MappingKit 默认使用的包名)
    val modelPackageName = "com.moshi.common.com.moshi.common.model"
    // com.moshi.common.model 文件保存路径 (MappingKit 与 DataDictionary 文件默认保存路径)
    ...
    val dbView = Engine.create("db_view")
      .getTemplate(
        PathKit.getWebRootPath() + "/src/main/java/com/moshi/common/com.moshi.common.model/_db_view.tpl"
      )
    val dbViewRaw = dbView.renderToString(
      Kv.by("tableMetas", tableMetas).set("suffixs", arrayOf("l", "t", "m"))
    )
    FileWriter(PathKit.getWebRootPath() + "/src/main/java/com/moshi/common/com.moshi.common.model/_db_view.sql")
      .write(dbViewRaw)
    ...
    val form = engine.getTemplate(
      PathKit.getWebRootPath() + "/src/main/java/com/moshi/common/com.moshi.common.model/_form.tpl"
    )
    for (meta in tableMetas) {
      ...
      FileWriter(
        PathKit.getWebRootPath()
            + "/src/main/java/com/moshi/common/com.moshi.common.model/_forms/"
    ...
    val configView = Engine.create("config_json")
      .getTemplate(
        PathKit.getWebRootPath() + "/src/main/java/com/moshi/common/com.moshi.common.model/_config_json.tpl"
      )
