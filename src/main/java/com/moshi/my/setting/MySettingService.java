...
//		String salt = account.getSalt();
		String hashedPass = HashKit.sha256( oldPassword);
		if ( ! hashedPass.equals(account.getPassword())) {
			return Ret.fail("msg", "原密码不正确，请重新输入");
		}

		newPassword = HashKit.sha256( newPassword);
		int result = Db.update("update account set password=? where id=? limit 1", newPassword,  accountId);
		if (result > 0) {
