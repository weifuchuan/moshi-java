...
public class SqlSrvLearnerInterceptor implements Interceptor {
  private WallFilter filter;

  public SqlSrvLearnerInterceptor() {
  ...
  private WallConfig buildConfig() {
    ...
    JSONObject jsonObject = JSON.parseObject(json).getJSONObject("learner");
    String[] keys =
        new String[] {
          "denyFunctions",
          "denyTables",
  ...
  public static void main(String[] args) throws SQLException {
    SqlSrvLearnerInterceptor interceptor = new SqlSrvLearnerInterceptor();
    interceptor.filter.check("select email from `v_account` a, role where a.id=? and role.id = a.id");
