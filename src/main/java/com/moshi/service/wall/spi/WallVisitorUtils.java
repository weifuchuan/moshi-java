 ...
 */
package com.moshi.service.wall.spi;
...
import com.moshi.service.wall.WallConfig.TenantCallBack;
import com.moshi.service.wall.WallConfig.TenantCallBack.StatementType;
import com.moshi.service.wall.*;
import com.moshi.service.wall.violation.ErrorCode;
import com.moshi.service.wall.violation.IllegalSQLObjectViolation;
...
public class WallVisitorUtils {
  ...
  public static void checkSelelct(WallVisitor visitor, SQLSelectQueryBlock x) {
    ...
    if (where != null) {
      ...
      Object whereValue =
          getConditionValue(visitor, where, visitor.getConfig().isSelectWhereAlwayTrueCheck());
  ...
  public static void checkHaving(WallVisitor visitor, SQLExpr x) {
    ...
    if (Boolean.TRUE
        == getConditionValue(visitor, x, visitor.getConfig().isSelectHavingAlwayTrueCheck())) {
  ...
  public static void checkDelete(WallVisitor visitor, SQLDeleteStatement x) {
    ...
    if (where != null) {
      ...
      if (Boolean.TRUE == getConditionValue(visitor, where, config.isDeleteWhereAlwayTrueCheck())) {
        if (config.isDeleteWhereAlwayTrueCheck()
            && visitor.isSqlEndOfComment()
  ...
  private static void checkCondition(WallVisitor visitor, SQLExpr x) {
    ...
    if (visitor.getConfig().isMustParameterized()) {
      ExportParameterVisitor exportParameterVisitor =
          visitor.getProvider().createExportParameterVisitor();
  ...
  private static void checkJoinSelectForMultiTenant(
      WallVisitor visitor, SQLJoinTableSource join, SQLSelectQueryBlock x) {
  ...
  @Deprecated
  public static void checkConditionForMultiTenant(
      WallVisitor visitor, SQLExpr x, SQLObject parent) {
    String tenantTablePattern = visitor.getConfig().getTenantTablePattern();
    ...
    SQLBinaryOpExpr tenantCondition =
        createTenantCondition(visitor, alias, statementType, matchTableName);
  ...
  @Deprecated
  public static void checkJoinConditionForMultiTenant(
      WallVisitor visitor,
    ...
    if (right instanceof SQLExprTableSource) {
      ...
      if (tableExpr instanceof SQLIdentifierExpr) {
        ...
        if (ServletPathMatcher.getInstance().matches(tenantTablePattern, tableName)) {
          ...
          SQLBinaryOpExpr tenantCondition =
              createTenantCondition(visitor, alias, statementType, tableName);

          if (condition == null) {
            condition = tenantCondition;
          } else {
            condition =
                new SQLBinaryOpExpr(tenantCondition, SQLBinaryOperator.BooleanAnd, condition);
          }
        }
      }
    }
  ...
  @Deprecated
  private static SQLBinaryOpExpr createTenantCondition(
      WallVisitor visitor, String alias, StatementType statementType, String tableName) {
    SQLExpr left, right;
    if (alias != null) {
      left =
          new SQLPropertyExpr(new SQLIdentifierExpr(alias), visitor.getConfig().getTenantColumn());
    } else {
  ...
  private static SQLExpr generateTenantValue(
      WallVisitor visitor, String alias, StatementType statementType, String tableName) {
    SQLExpr value;
      ...
      if (isUpdateCheckTable && updateCheckHandler != null) {
        ...
        if (valueExpr != null) {
          ...
          if (where instanceof SQLBinaryOpExpr) {
            conditions =
                SQLBinaryOpExpr.split((SQLBinaryOpExpr) where, SQLBinaryOperator.BooleanAnd);
            ...
            for (SQLExpr each : ((SQLBinaryOpExprGroup) where).getItems()) {
              if (each instanceof SQLBinaryOpExpr) {
                conditions.addAll(
                    SQLBinaryOpExpr.split((SQLBinaryOpExpr) each, SQLBinaryOperator.BooleanAnd));
          ...
          if (allValue) {
            ...
            boolean validate =
                updateCheckHandler.check(tableName, checkColumn, setValue, filterValues);
            if (!validate) {
              visitor.addViolation(
                  new IllegalSQLObjectViolation(
                      ErrorCode.UPDATE_CHECK_FAIL, "update check failed.", visitor.toSQL(x)));
            }
          } else {
            visitor.addWallUpdateCheckItem(
                new WallUpdateCheckItem(tableName, checkColumn, valueExpr, filterValueExprList));
          }
    ...
    boolean checkCondition =
        visitor != null
            && (!visitor.getConfig().isConstArithmeticAllow()
                || !visitor.getConfig().isConditionOpBitwseAllow()
                || !visitor.getConfig().isConditionOpXorAllow());
  ...
  public static Object getValueFromAttributes(WallVisitor visitor, SQLObject sqlObject) {
    ...
    if (visitor != null
        && visitor.getConfig().isConditionLikeTrueAllow()
    ...
    if (x instanceof SQLQueryExpr) {
      ...
      if (isSimpleCaseTableSource(visitor, ((SQLQueryExpr) x).getSubQuery())) {
        SQLSelectQueryBlock queryBlock =
            (SQLSelectQueryBlock) ((SQLQueryExpr) x).getSubQuery().getQuery();
  ...
  public static Object eval(
      WallVisitor wallVisitor, String dbType, SQLObject sqlObject, List<Object> parameters) {
    ...
    if (query instanceof SQLSelectQueryBlock) {
      ...
      if (queryBlock.getSelectList().size() == 1) {
        ...
        if (selectItemExpr instanceof SQLAggregateExpr) {
          if (((SQLAggregateExpr) selectItemExpr).methodNameHashCod64()
              == FnvHash.Constants.COUNT) {
            simpleCount = true;
          }
        }
      }
  ...
  private static boolean checkSchema(WallVisitor visitor, SQLExpr x) {
    ...
    if (x instanceof SQLName) {
      ...
      if (isInTableSource(x) && !visitor.getProvider().checkDenySchema(owner)) {
        ...
        if (!isTopStatementWithTableSource(x)
            && !isFirstSelectTableSource(x)
            && !isFirstInSubQuery(x)) {
          SQLObject parent = x.getParent();
  ...
  public static boolean check(WallVisitor visitor, SQLExprTableSource x) {
    ...
    if (expr instanceof SQLName) {
      ...
      if (!visitor.getConfig().getPermitTables().contains(WallVisitorUtils.form(tableName))) {
        addViolation(visitor, ErrorCode.OTHER, "not in permit tables : " + tableName, x);
        return false;
      }
    }
  ...
  private static void addViolation(
      WallVisitor visitor, int errorCode, String message, SQLObject x) {
  ...
  public static void checkUnion(WallVisitor visitor, SQLUnionQuery x) {
    ...
    if (!WallVisitorUtils.queryBlockFromIsNull(visitor, x.getLeft())
      ...
      if (((x.getOperator() == SQLUnionOperator.UNION
              ...
              && visitor.getConfig().isSelectUnionCheck()
              && visitor.isSqlEndOfComment())
          || (x.getOperator() == SQLUnionOperator.MINUS && visitor.getConfig().isSelectMinusCheck())
          || (x.getOperator() == SQLUnionOperator.INTERSECT
              && visitor.getConfig().isSelectIntersectCheck())
          || (x.getOperator() == SQLUnionOperator.EXCEPT
              && visitor.getConfig().isSelectExceptCheck())) {
        addViolation(
            visitor,
            ErrorCode.UNION,
            x.getOperator().toString() + " query not contains 'from clause'",
            x);
      }
    }
  }
  ...
  public static boolean queryBlockFromIsNull(
