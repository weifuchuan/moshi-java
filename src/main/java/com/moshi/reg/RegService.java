...
import static com.moshi.login.LoginService.loginAccountCacheName;
import static com.moshi.login.LoginService.sessionIdName;
...
public class RegService {
  ...
  public boolean isNickNameExists(String nickName) {
    ...
    return Db.queryInt("select id from account where lower(nickName) = ? limit 1", nickName)
        != null;
  }
  ...
  public Ret activate(String authCodeId) {
    ...
    if (authCode != null && authCode.isValidRegActivateAuthCode()) {
      ...
      int n =
          Db.update(
              "update account set status = ? where id = ? and status = ?",
              Account.STATUS_LEANER,
              authCode.get("accountId"),
              Account.STATUS_REG);
      if (n > 0) {
  ...
  public Ret reSendActivateEmail(String email) {
    ...
    Account account =
        accountDao.findFirst(
            "select * from account where email=? and status = ? limit 1",
            email,
            Account.STATUS_REG);
    if (account == null) {
