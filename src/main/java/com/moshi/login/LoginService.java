...
public class LoginService {
  ...
  public Ret loginByEmail(String email, String password) {
    ...
    Account account = accountDao.findFirst("select id from account where email = ? ", email);
    ...
    return Ret.ok(sessionIdName, sessionId).set(loginAccountCacheName, account)
        .set("maxAgeInSeconds", maxAgeInSeconds); // 用于设置
                                                  // cookie
                                                  // 的最大存活时间
  }
  ...
  public void reloadLoginAccount(Account loginAccountOld) {
    ...
    // 更新缓存只需要 CacheKit.remove("account", id) 就可以了，
